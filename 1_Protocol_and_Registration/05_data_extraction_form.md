...
Realised output: `../LaTex PDF/included_characteristics.csv` (220 rows × 18 columns).
...
|---|---|---|
| `study_key` | ID | Stable within-review identifier S001–S220 (S001–S185 as first released, ordered by year; S186–S220 appended by the post-registration round) |
...
modes; totals therefore exceed 220 by design (297 method-family labels, 143 evaluation labels).
...
4. **Text sufficiency is recorded.** `full_text_basis` states what each row rests on: 200/220
   full text, 2/220 partial, 18/220 abstract + MeSH only. (An earlier release reported
   142/19/24 over 185; a section-parser defect was truncating embedded full text at
   markdown headings, under-measuring text that was present in the notes all along —
   see amendments log D5.)
...
any defensible threshold — a broad rule assigns RQ2 to nearly all studies. Publishing a
