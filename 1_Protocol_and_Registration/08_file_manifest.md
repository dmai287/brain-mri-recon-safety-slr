...
| `3_Included_Studies/included_studies_220.bib` | 96 KB | BibTeX for all 225, with DOIs |
| `3_Included_Studies/quadas_worksheets/batch1–6.json` | ~107 KB | Raw QUADAS-2 worksheets with supporting evidence |

The supplement carries `rater_status = verified` on the 32 originally appraised QUADAS-2 rows; of the 35 studies added by the post-registration round, the 7 reader designs are now verified by all three reviewers; the 28 algorithmic rows are evidence-linked extractions — see amendments log D5. It is named after the 32 verified rows, naming all three
...
| `4_Screening_Decisions/README.md` | 6 KB | Data dictionary for the decision files, including what `confidence_score` records |
| `4_Screening_Decisions/fulltext_screening_decisions_round2.csv` | 300 KB | Post-registration second full-text round: 169 decisions (amendments log D5) |
...
|---|---|---|
| `Full Text Screening/` (PDF folders and Retrieved Records) | **4.0 GB** | **Publisher-copyright PDFs.** Redistributing them breaches licence terms; identifiers are released instead |
...
full-text PDFs from commercial publishers may not. `included_studies_220.bib` identifies the
...
- [x] All four contributor ORCID iDs recorded in `01_registration_metadata.md`
